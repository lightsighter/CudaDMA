...
def main():
  ...
  for i in warp_counts:
    dma_sz = 4*32*compute_warps
    for j in range(32*4, dma_sz+1, 32*4):
        el_sz = j
        if dma_sz % el_sz != 0: continue
        ...
        f.write("#define EL_SZ " + str(el_sz) + "\n")
        f.close()

        ########
        # Run test
        ########
        os.system('make clean ; make x86_64=1 keep=1 ; ../../bin/linux/release/saxpy_cudaDMA_strided')
