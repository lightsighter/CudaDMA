...
def main():
  ...
  warp_counts = [1,2,3]
  #warp_counts = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 14, 16, 18, 20, 22, 24]
  for i in warp_counts:
    ...
    ########
    f = open(outfile,'w')
    f.write("#define SAXPY_KERNEL saxpy_shmem_doublebuffer\n")
    f.write("#define CTA_COUNT 14\n")
    f.write("#define COMPUTE_THREADS_PER_CTA 32 * " + str(i) + "\n")
    f.write("#define NUM_ITERS 2048\n")
    f.write("#define DMA_THREADS_PER_LD 32 * " + str(i) + "\n")
    #f.write("#define BYTES_PER_DMA_THREAD ( 4 * ( (COMPUTE_THREADS_PER_CTA+DMA_THREADS_PER_LD-1)/DMA_THREADS_PER_LD ) )\n")
